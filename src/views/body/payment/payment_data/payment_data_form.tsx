...
import {saveAddress, saveCreditCard} from "../../../../api/user";

interface PaymentDataForm {
    onPayClick: (creditCard: CreditCard) => void;
    onBackClick: () => void;
}

function PaymentDataForm({onPayClick, onBackClick}: PaymentDataForm) {
    ...
    const [showCcvNumberError, setShowCcvNumberError] = useState(false);
    ...
    const [saveCreditCardState, setSaveCreditCardState] = useState<boolean>(false);

    const handleSaveCreditCardCheckboxChange = (event: ChangeEvent<HTMLInputElement>) => {
        setSaveCreditCardState(event.target.checked);
    };
    ...
    const handleCcvNumberChange = (event: ChangeEvent<HTMLInputElement>) => {
        const input = event.target.value.replace(/\D/g, '');
        setShowCcvNumberError(false)
        setCcvNumber(input);
    };

    const handleExpDateChange = (event: ChangeEvent<HTMLInputElement>) => {
        const input = event.target.value.replace(/\D/g, '');
        const formattedInput = input.replace(/(.{2})/, '$1/');
    ...
    const handlePayClick = () => {
        ...
        if (expDate.length !== 5 || !isValidExpiryDate(expDate)) {
            setShowExpDateError(true)
            hasError = true
        }
        if (ccvNumber.length !== 3) {
            setShowCcvNumberError(true)
            hasError = true
        }
        ...
        if (!hasError) {
            const creditCard: CreditCard = new CreditCard(cardNumber, ccvNumber, expDate, cardName);
            if (saveCreditCardState && authToken != null) {
                saveCreditCard(creditCard)
            }
            onPayClick(creditCard);
        }
    };
    ...
    const turnCard = () => {
        setSeeBack((prevSeeBack) => !prevSeeBack);
    };
    ...
    function isValidExpiryDate(dateString: String) {
        ...
        return month >= 1 && month <= 12 && year >= 21;
    }

    return (
        <div className="w-[750px] flex flex-col">
            <form
                className="w-full flex flex-col lg:flex-row items-center justify-center mb-4"
            >
                <div className="max-w-[320px] lg:pr-8 lg:border-r-2 lg:border-slate-300">
                    <div className="mb-4">
                        <input
                            id="cardNumber"
                            type="text"
                            className={`${showCardNumberError ? 'border-red-600 border-4 ' : 'border'}rounded-md border h-12 w-full py-2 px-3 text-gray-700 leading-tight focus:outline-1 focus:outline-blue-500`}
                            maxLength={19}
                            placeholder="Номер картки"
                            value={cardNumber}
                            onChange={handleCardNumberChange}
                            onClick={showFrontCard}
                        />
                    </div>
                    <div className="flex gap-x-2 mb-4">
                        <div className="block">
                            <input
                                id="expDate"
                                type="text"
                                className={`${showExpDateError ? 'border-red-600 border-4 ' : 'border'}rounded-md border h-12 w-full py-2 px-3 text-gray-700 leading-tight focus:outline-1 focus:outline-blue-500`}
                                maxLength={5}
                                placeholder="MM/YY"
                                value={expDate}
                        ...
                        <div className="block">
                            <input
                                id="ccvNumber"
                                type="text"
                                className={`${showCcvNumberError ? 'border-red-600 border-4 ' : 'border'} rounded-md border h-12 w-full py-2 px-3 text-gray-700 leading-tight focus:outline-1 focus:outline-blue-500`}
                                maxLength={3}
                                placeholder="CCV"
                                value={ccvNumber}
                    ...
                    <div>
                        <input
                            id="cardName"
                            type="text"
                            className={`${showCardNameError ? 'border-red-600 border-4 ' : 'border'} rounded-md border h-12 w-full py-2 px-3 text-gray-700 leading-tight focus:outline-1 focus:outline-blue-500`}
                ...
                <div className="min-w-[430px] lg:pl-8">
                    <div className="w-full max-w-sm h-56">
                        <div
                            id="creditCard"
                            className={`${seeBack ? 'rotated ' : ''} relative creditCard cursor-pointer transition-transform duration-300`}
                            onClick={turnCard}
                        >
                            {!seeBack && (
                                <div
                                    ...
                                    <div className="w-full px-8 absolute top-8">
                                        ...
                                        <div className="pt-6 flex justify-between">
                                            <div>
                                                ...
                                                <p
                                                    id="imageCardName"
                                                    className="font-medium tracking-widest h-6 line-clamp-1"
                                                >
                                                    {cardName}
                                                </p>
                                            </div>
                            ...
                            {seeBack && (
                                <div
                                    className="w-full h-56 m-auto rounded-xl text-white shadow-2xl absolute rotated"
                                >
                                    <img
                                        src="https://i.ibb.co/LPLv5MD/Payment-Card-01.jpg"
                                        className="relative object-cover w-full h-full rounded-xl"
            ...
            {authToken != null && (
                <div className="flex items-center mb-4">
                    <input id="save-credit-card-checkbox" type="checkbox" value="" onChange={handleSaveCreditCardCheckboxChange}
                           className="w-5 h-5 border rounded-md"/>
                    <label htmlFor="save-credit-card-checkbox" className="ms-2 text-sm font-medium text-gray-900">
